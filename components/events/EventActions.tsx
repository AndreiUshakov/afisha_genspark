...
export default function EventActions({ eventId, eventTitle, initialFavorited = false }: EventActionsProps) {
  ...
  const handleShare = (platform: string) => {
    ...
    const text = `Посмотрите это мероприятие: ${eventTitle}`;
  ...
  return (
    <div className="flex flex-wrap gap-3">
      {/* Register/Buy Button */}
      <button
        className="flex-1 min-w-[200px] px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors"
        onClick={() => alert('Регистрация на мероприятие будет реализована позже')}
      >
        Зарегистрироваться
      </button>
