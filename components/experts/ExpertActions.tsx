...
export default function ExpertActions({ 
  ...
  const handleSubscribeClick = () => {
    ...
    if (!isSubscribed) {
      console.log('Subscribed to expert:', expertId);
    } else {
      console.log('Unsubscribed from expert:', expertId);
    }
  };

  const handleContactClick = () => {
    // TODO: Implement contact functionality
    alert('Функция связи с экспертом будет реализована позже');
  };

  const handleShare = (platform: string) => {
    const url = window.location.href;
    const text = `Рекомендую эксперта: ${expertName}`;
