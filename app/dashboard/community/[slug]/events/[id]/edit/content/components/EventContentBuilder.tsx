...
import { 
  ...
  deleteEventContentBlock, 
  reorderEventContentBlocks 
} from '../actions'
import HeadingBlockEditor from '@/app/dashboard/community/[slug]/settings/content/components/HeadingBlockEditor'
import TextBlockEditor from '@/app/dashboard/community/[slug]/settings/content/components/TextBlockEditor'
import ImageBlockEditor from '@/app/dashboard/community/[slug]/settings/content/components/ImageBlockEditor'
import CarouselBlockEditor from '@/app/dashboard/community/[slug]/settings/content/components/CarouselBlockEditor'
...
export default function EventContentBuilder({ 
  eventId, 
  communityId, 
  communitySlug, 
  initialBlocks 
}: EventContentBuilderProps) {
  ...
  const handleAddBlock = async (blockType: BlockType) => {
    ...
    try {
      ...
      const result = await createEventContentBlock(eventId, {
  ...
  const handleUpdateBlock = async (blockId: string, content: BlockContent) => {
    ...
    try {
      const result = await updateEventContentBlock(eventId, {
        id: blockId,
        content
      })
  ...
  const handleDeleteBlock = async (blockId: string) => {
    ...
    try {
      const result = await deleteEventContentBlock(eventId, blockId)
  ...
  const handleMoveBlock = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const result = await reorderEventContentBlocks(eventId, blockIds)
      ...
      {blocks.length > 0 ? (
        ...
        <div className="text-center py-12 bg-gray-50 dark:bg-neutral-900 rounded-lg border-2 border-dashed border-gray-300 dark:border-neutral-700">
          ...
          <p className="text-gray-600 dark:text-neutral-400 mb-4">
            Контент события пока пуст. Добавьте первый блок!
          </p>
        </div>
      )}
