...
import { updateEventBasic, type CreateEventBasicData } from '../../../../create/actions'
...
interface BasicEventEditFormProps {
  event: Event
  community: Community
  categories: Category[]
...
export default function BasicEventEditForm({ event, community, categories, communitySlug }: BasicEventEditFormProps) {
  const router = useRouter()
  const [isSaving, setIsSaving] = useState(false)
  const [message, setMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null)
  const [errors, setErrors] = useState<Record<string, string>>({})

  // Инициализация формы с данными мероприятия
  const [formData, setFormData] = useState<CreateEventBasicData>({
    title: event.title,
    description: event.description,
    category_id: event.category_id,
    start_date: event.start_date.slice(0, 16), // Формат для datetime-local
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateEventBasic(event.id, communitySlug, formData)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-gray-200 dark:border-neutral-700 p-6">
        ...
        <p className="text-sm text-gray-600 dark:text-neutral-400 mb-6">
          Настройте параметры фильтрации для этого мероприятия
        </p>
      ...
      <div className="flex justify-between items-center">
        <button
          type="button"
          onClick={() => router.push(`/dashboard/community/${communitySlug}/events/${event.id}/edit`)}
